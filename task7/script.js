...
scriptModule = (function () {
    ...
    let postsList = new PostsList(JSON.parse(localStorage.getItem('posts')));
        ...
        if (user === 'guest') {
            ...
            if (user === photoPost.author)
                ...
                if (index === -1)
                    ...
                    HTML += '</div>\n<div class="like-btn" style="background: url(dog-icon-like.png)" onclick="scriptModule.like(' + photoPost.id + ')"></div>';
            }
        }
        return HTML;
    };
    ...
    let editPost = (id) => {
        id = id.toString();
        localStorage.setItem('editPost', JSON.stringify(postsList.get(id)));
    ...
    let like = (id) => {
        ...
        if (like.style.background === 'url("dog-icon-like.png")') {
            like.style.background = 'url("dog-icon.png")';
            let tmp = postsList.get(id);
            ...
            like.style.background = 'url("dog-icon-like.png")';
            let tmp = postsList.get(id);
            tmp.likes.push(user);
            localStorage.removeItem('posts');
            localStorage.setItem('posts', JSON.stringify(postsList.array));
